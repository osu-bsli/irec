...
static esp_err_t read_registers(struct fc_adxl375 *device, uint8_t reg,
...
static esp_err_t write_registers(struct fc_adxl375 *device, uint8_t reg, uint8_t *data, uint8_t length)
{
  Wire.beginTransmission((uint8_t)I2C_ADDRESS);
  Wire.write(reg);
...
static esp_err_t is_data_ready(struct fc_adxl375 *device, int *isready)
{
  uint8_t interrupt_data;

  /* read INT_SOURCE bits (pg. 23) */
  esp_err_t status = read_registers(
...
esp_err_t fc_adxl375_initialize(struct fc_adxl375 *device)
{
  /* reset struct */
