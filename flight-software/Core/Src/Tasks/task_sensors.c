...
/*
 * FC TO RADIO COMMS via SLIP PROTOCOL
 */

static uint8_t packet_tx_buf[256];
static uint8_t slip_buf[256];
static uint8_t slip_tx_buf[256];
static uint8_t slip_tx_buf_i = 0;
...
{
  _Static_assert(sizeof(slip_tx_buf) == 256);
  slip_tx_buf[slip_tx_buf_i] = byte;
  slip_tx_buf_i++;
  return 1;
}
...
static void task_sensors(void *argument)
{
  ...
  float ms5607_pressure_mbar_min = 0.0;
  ...
  while (true)
  {
    ...
    if (ms5607_pressure_mbar_min == 0.0 || ms5607_data.pressure_mbar < ms5607_pressure_mbar_min)
    {
      ms5607_pressure_mbar_min = ms5607_data.pressure_mbar;
    }
    ...
    if (sdcard_get_is_in_degraded_state())
      status_flags |= STATUS_FLAGS_SD_CARD_DEGRADED;
    ...
    if (HAL_UART_GetState(&huart6) == HAL_UART_STATE_READY)
    {
      ...
      // Copy from SLIP buffer into final TX buffer
      memcpy(packet_tx_buf, &slip_tx_buf, slip_tx_buf_i);

      // TX it.
      HAL_UART_Transmit_IT(&huart6, packet_tx_buf, slip_tx_buf_i);
