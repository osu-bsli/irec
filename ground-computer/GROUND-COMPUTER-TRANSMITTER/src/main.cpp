...
telemetry_packet getNextDataPacket() {
  String str = FlightData.readStringUntil('\n');
  telemetry_packet temp;
  temp.ms5607_pressure_mbar = getStringPartByNr(str, ',', 0).toFloat();
  return temp;
}
...
void setup() {
  ...
  if(!SD.begin(9,SPI_HALF_SPEED,SPI1)) {
    ...
    if(!SD.exists("/TransmissionData.csv")) {
      Serial.println("Couldn't find file, ending program");
      while(1) {
        delay(1000);
        Serial.println("Kill Program");
      }
    }
    FlightData = SD.open("/TransmissionData.csv");
    FlightData.readStringUntil('\n'); //get rid of first line (definitions)
  }
}
...
void loop() {
  ...
  delay(10);
  Serial.println("Sending next data packet");
  // 2. CHECK Serial Monitor for outgoing text
    //Read the string until newline
  telemetry_packet npacket =  getNextDataPacket();
  uint8_t data[sizeof(BrianTag) + sizeof(npacket)];
  memcpy(data, &BrianTag, sizeof(BrianTag));
  memcpy(&data[sizeof(BrianTag)], &npacket, sizeof(npacket));
