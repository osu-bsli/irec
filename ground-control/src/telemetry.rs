...
impl<T: Clone + bytemuck::AnyBitPattern> TelemetryDecoder<T> {
    ...
    pub fn decode(&mut self, byte: u8) -> Option<T> {
        if self.data_pos < self.magic.len() {
            self.data[self.data_pos] = byte;

            if byte == self.magic[self.data_pos] {
                self.data_pos += 1;
            } else {
                self.data_pos = 0;
            }
            ...
            if self.data_pos >= size_of::<T>() {
                ...
                let header = bytemuck::from_bytes_mut::<PacketHeader>(&mut self.data);
                ...
                let packet = bytemuck::from_bytes::<T>(&self.data);
