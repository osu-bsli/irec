...
sindatay2 = []
for i in range(0, 10000):
    sindatay2.append(2 + 0.5 * sin(50 * i / 10000))
...
with dpg.window(label="Tutorial", width=1200, height=800):
    ...
    with dpg.plot(tag="plot", label="Line Series", height=-1, width=-1):
        ...
        dpg.add_line_series(sindatax, sindatay, label="0.5 + 0.5 * sin(x)", parent="yaxis", tag="series_data")
        dpg.add_line_series(sindatax, sindatay2, label="2 + 0.5 * sin(x)", parent="yaxis", tag="series_data2")
